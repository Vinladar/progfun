...
object Huffman {
  ...
  def createCodeTree(chars: List[Char]): CodeTree =
    (times _ andThen makeOrderedLeafList _ andThen until(singleton, combine) _ andThen head _)(chars)
 
  // Part 3: Decoding

  type Bit = Int

  implicit def bitToBoolean(bit:Bit):Boolean = bit == 1
    ...
    def decode1(tree: CodeTree, bits: List[Bit]): List[Char] =
      (bits, tree) match {
        case (bit::rest, Fork(left: CodeTree, right: CodeTree, chars: List[Char], weight: Int)) =>
          if(bit) decode1(right, rest) else decode1(left, rest)
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.find(p=>p._1==char).map(p=>p._2).getOrElse(Nil)
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    case Fork(left, right, c, weight) =>
      mergeCodeTables(('0',0::Nil)::convert(left),('1',1::Nil)::convert(right))
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable =
    ...
    }) ++ b.foldLeft(List(('0', List[Bit]())))((acc, n) => {
      n match {
        case (char, Nil) => acc.map(x=>(char, x._2))
        case (char, lb) => acc.map(x=>(char, lb ++ x._2))
      }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] =
    text.foldLeft(List[Bit]())((acc: List[Bit], char: Char)=>acc ++ codeBits(convert(tree))(char))
  }
